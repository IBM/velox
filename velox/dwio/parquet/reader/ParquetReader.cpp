...
ParquetReaderFactory::createFormatOptions(
  ...
  options->nullStructForMissingFields =
      ParquetConfig::nullStructForMissingFields(connectorConfig, session);
...
class ReaderBase {
 public:
  ...
  bool nullStructForMissingFields() const {
    return parquetReaderOptions_.nullStructForMissingFields;
  }
...
class ParquetRowReader::Impl {
 public:
  Impl(
    ...
    columnReaderOptions_.parquetNullStructForMissingFields_ =
        readerBase_->nullStructForMissingFields();
