...
// Returns how many of the rows in [begin, end) selected by 'mask' (every row
// when 'mask' is nullptr) are not null according to 'source'. Used to find out
// whether a partition has any null at all before a bitmap is allocated for it.
int32_t countSelectedNotNull(
    const uint64_t* source,
    const uint64_t* mask,
    int32_t begin,
    int32_t end) {
  if (mask == nullptr) {
    return bits::countBits(source, begin, end);
  }
  ...
  bits::forEachWord(
      ...
      [&](int32_t index, uint64_t wordMask) {
        count += __builtin_popcountll(source[index] & mask[index] & wordMask);
      },
      [&](int32_t index) {
        count += __builtin_popcountll(source[index] & mask[index]);
      });
  return count;
}
...
void PrestoIterativePartitioningSerializer::flushNulls(
  ...
  // (1 == not null). Allocated on demand: a partition whose rows are all
  // non-null writes no null section, so it never needs a bitmap and a column
  // without nulls allocates nothing at all.
  std::vector<vector_size_t> nullCounts(numPartitions_, 0);
  ...
  // Allocates the bitmap of partition 'p' on the first null seen there and
  // marks the rows written so far as not null. Those rows are all not null,
  // otherwise the bitmap would already exist. The bitmap gets one extra word so
  // the bit appender in compactBits can always touch the word past the last
  // bit.
  auto ensureNulls = [&](uint32_t p) {
    if (rawNulls[p] != nullptr) {
      return;
    }
    const auto numWords = bits::nwords(context.rowCounts[p]) + 1;
    ...
    bits::fillBits(
        rawNulls[p], 0, static_cast<int32_t>(bitOffsets[p]), bits::kNotNull);
  };
  ...
  for (size_t vectorIndex = 0; vectorIndex < numVectors; ++vectorIndex) {
    ...
    for (uint32_t p : nonEmptyPartitions) {
      ...
      if (allNull) {
        ...
        // No nulls in this batch: mark all present rows not null, needed only
        // if an earlier batch already forced a bitmap for this partition.
        if (rawNulls[p] != nullptr) {
          bits::fillBits(
              rawNulls[p],
              bitOffsets[p],
        ...
        const auto valid =
            countSelectedNotNull(validBits, parentNulls, begin, end);
        if (valid < present) {
          ensureNulls(p);
