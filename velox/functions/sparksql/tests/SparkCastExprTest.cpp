...
class SparkCastExprTest : public functions::test::CastBaseTest {
 protected:
  ...
  void testPrimitiveInvalidCornerCase() {
    // This method is intentionally empty in the base implementation.
    // Invalid corner cases are tested separately in ANSI-specific tests.
  }
...
TEST_F(SparkCastExprTestAnsiOn, testInvalidDate) {
  ...
  testInvalidCast<std::string>(
      "date", {"2012-Oct-23"}, "Invalid date format", VARCHAR());
  testInvalidCast<std::string>(
      "date", {"2015-03-18X"}, "Invalid date format", VARCHAR());
  testInvalidCast<std::string>(
      "date", {"2015/03/18"}, "Invalid date format", VARCHAR());
  testInvalidCast<std::string>(
      "date", {"2015.03.18"}, "Invalid date format", VARCHAR());
  testInvalidCast<std::string>(
      "date", {"20150318"}, "Invalid date format", VARCHAR());
  testInvalidCast<std::string>(
      "date", {"2015-031-8"}, "Invalid date format", VARCHAR());
  testInvalidCast<std::string>(
      "date", {"-1-1-1"}, "Invalid date format", VARCHAR());
  testInvalidCast<std::string>(
      "date", {"-11-1-1"}, "Invalid date format", VARCHAR());
  testInvalidCast<std::string>(
      "date", {"-111-1-1"}, "Invalid date format", VARCHAR());
  testInvalidCast<std::string>(
      "date", {"- 1111-1-1"}, "Invalid date format", VARCHAR());
}
...
TEST_F(SparkCastExprTestAnsiOn, doubleToTimestamp) {
  ...
  testInvalidCast<double>(
      "timestamp", {kInf}, "Cannot cast infinity to TIMESTAMP");
  testInvalidCast<double>("timestamp", {kNan}, "Cannot cast NaN to TIMESTAMP");
  testInvalidCast<double>(
      "timestamp", {-kInf}, "Cannot cast infinity to TIMESTAMP");
}
...
TEST_F(SparkCastExprTestAnsiOn, floatToTimestamp) {
  ...
  testInvalidCast<float>(
      "timestamp", {kInf}, "Cannot cast infinity to TIMESTAMP");
  testInvalidCast<float>("timestamp", {kNan}, "Cannot cast NaN to TIMESTAMP");
  testInvalidCast<float>(
      "timestamp", {-kInf}, "Cannot cast infinity to TIMESTAMP");
}

TEST_F(SparkCastExprTestAnsiOn, primitiveInvalidCornerCase) {
  testPrimitiveInvalidCornerCase();

  // To floating-point - invalid strings should throw
  testInvalidCast<std::string>("real", {"1.2a"}, "Invalid input for cast");
  testInvalidCast<std::string>("real", {"1.2.3"}, "Invalid input for cast");

  // To boolean - invalid strings should throw
  testInvalidCast<std::string>(
      "boolean", {"1.7E308"}, "Invalid input for cast");
  testInvalidCast<std::string>("boolean", {"nan"}, "Invalid input for cast");
  testInvalidCast<std::string>("boolean", {"12"}, "Invalid input for cast");
  testInvalidCast<std::string>("boolean", {"-1"}, "Invalid input for cast");
  testInvalidCast<std::string>("boolean", {"tr"}, "Invalid input for cast");
  testInvalidCast<std::string>("boolean", {"tru"}, "Invalid input for cast");
  testInvalidCast<std::string>("boolean", {"on"}, "Invalid input for cast");
  testInvalidCast<std::string>("boolean", {"off"}, "Invalid input for cast");
}
...
TEST_F(SparkCastExprTestAnsiOn, overflow) {
  ...
  testInvalidCast<std::string>("tinyint", {"166"}, "out of range");
  testInvalidCast<std::string>("smallint", {"52769"}, "out of range");
  testInvalidCast<std::string>("integer", {"17515055537"}, "out of range");
  testInvalidCast<std::string>("integer", {"-17515055537"}, "out of range");
  testInvalidCast<std::string>(
      "bigint", {"9663372036854775809"}, "out of range");
}
