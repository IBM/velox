...
void HashBuild::addRuntimeStats() {
  ...
  for (const auto& timing : tableBuilder_->table()->parallelJoinBuildStats().partitionTimings) {
  ...
  for (const auto& timing : tableBuilder_->table()->parallelJoinBuildStats().buildTimings) {
    lockedStats->getOutputTiming.add(timing);
    lockedStats->addRuntimeStat(
  ...
  for (const auto& timing :
       tableBuilder_->table()->parallelJoinBuildStats().bloomFilterPartitionTimings) {
    lockedStats->getOutputTiming.add(timing);
  ...
  for (const auto& timing :
       tableBuilder_->table()->parallelJoinBuildStats().bloomFilterBuildTimings) {
    lockedStats->getOutputTiming.add(timing);
    if (timing.wallNanos > 0) {
      lockedStats->addRuntimeStat(
