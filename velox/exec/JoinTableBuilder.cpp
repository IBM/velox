...
JoinTableBuilder::JoinTableBuilder(Options options)
  ...
  tableType_ = hashJoinTableType(
      options_.joinKeys, options_.inputType, dropDuplicates_);
}
...
void JoinTableBuilder::resetForSpillInput() {
  ...
  std::iota(
      dependentChannels_.begin(), dependentChannels_.end(), keyChannels_.size());
