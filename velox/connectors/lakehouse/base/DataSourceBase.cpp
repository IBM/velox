...
DataSourceBase::DataSourceBase(
  ...
  for (const auto& [canonicalizedName, columnHandle] : columnHandles) {
    ...
    switch (handle->columnType()) {
      ...
      default:
        break;
        //      case ColumnHandleBase::ColumnType::kHiveRowIndex:
        //        specialColumns_.rowIndex = handle->name();
        //        break;
        //      case ColumnHandleBase::ColumnType::kHiveRowId:
  ...
  if (ConnectorConfigBase_->isFileColumnNamesReadAsLowerCase(
...
//        infoColumns_,
//        specialColumns_,
//        ConnectorConfigBase_->readStatsBasedFilterReorderDisabled(
...
std::optional<RowVectorPtr> DataSourceBase::next(
    uint64_t size,
  ...
  TestValue::adjust(
      "facebook::velox::connector::lakehouse::base::DataSourceBase::next",
      this);
...
std::shared_ptr<velox::common::ScanSpec> DataSourceBase::makeScanSpec() {
  ...
  if (ConnectorConfigBase_->readStatsBasedFilterReorderDisabled(
          connectorQueryCtx_->sessionProperties())) {
