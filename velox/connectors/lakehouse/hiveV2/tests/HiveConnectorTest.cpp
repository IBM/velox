...
using namespace facebook::velox::connector::lakehouse::base;
...
groupSubfields(const std::vector<Subfield>& subfields) {
  ...
  for (auto& subfield : subfields) {
    auto& name = static_cast<const velox::common::Subfield::NestedField&>(
                     *subfield.path()[0])
                     .name();
...
// TODO:: Rewrite these ScanSpec related tests
