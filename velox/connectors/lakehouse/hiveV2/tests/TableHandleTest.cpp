...
TEST(FileHandleTest, hiveColumnHandle) {
  Type::registerSerDe();
  connector::lakehouse::hive::HiveColumnHandle::registerSerDe();
  auto columnType = ROW(
      {{"c0c0", BIGINT()},
       {"c0c1",
        ARRAY(MAP(
            VARCHAR(), ROW({{"c0c1c0", BIGINT()}, {"c0c1c1", BIGINT()}})))}});
  auto columnHandle = HiveConnectorTestBase::makeHiveColumnHandle(
  ...
  auto clone =
      ISerializable::deserialize<connector::lakehouse::hive::HiveColumnHandle>(obj);
