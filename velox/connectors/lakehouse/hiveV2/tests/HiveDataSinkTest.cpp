...
using namespace facebook::velox::connector::lakehouse::hive::test;
...
class HiveDataSinkTest : public HiveConnectorTestBase {
 protected:
  ...
  createHiveInsertTableHandle(
      ...
      const std::shared_ptr<connector::lakehouse::hive::HiveBucketProperty>&
  ...
  void verifyWrittenData(const std::string& dirPath, int32_t numFiles = 1) {
    ...
    HiveConnectorTestBase::assertQuery(
        exec::test::PlanBuilder().tableScan(rowType_).planNode(),
        splits,
