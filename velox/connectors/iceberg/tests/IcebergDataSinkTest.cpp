...
#include "velox/connectors/iceberg/IcebergSplit.h"
#include "velox/connectors/iceberg/tests/IcebergConnectorTestBase.h"
...
#include "velox/velox/connectors/hive/tests/IcebergConnectoTestBase.h"
...
class IcebergDataSinkTest : public connector::iceberg::test::IcebergConnectorTestBase,
                            public testing::WithParamInterface<std::tuple<
                                dwio::common::FileFormat,
                                std::optional<std::vector<std::string>>,
                                size_t>>,
                            public IcebergConnectoTestBase {
 protected:
  void SetUp() override {
  ...
  std::shared_ptr<connector::iceberg::IcebergInsertTableHandle>
  createIcebergInsertTableHandle(
      const RowTypePtr& outputRowType,
      const std::string& outputDirectoryPath,
      dwio::common::FileFormat fileFormat = dwio::common::FileFormat::DWRF,
      const std::vector<std::string>& partitionedBy = {}) {
    std::vector<std::shared_ptr<const connector::iceberg::IcebergColumnHandle>>
        columnHandles;
    ...
    for (int i = 0; i < columnNames.size(); ++i) {
      if (std::find(
        ...
        columnHandles.push_back(
            std::make_shared<connector::iceberg::IcebergColumnHandle>(
                columnNames.at(i),
                connector::iceberg::IcebergColumnHandle::ColumnType::kPartitionKey,
                columnTypes.at(i),
                columnTypes.at(i)));
      } else {
        columnHandles.push_back(
            std::make_shared<connector::iceberg::IcebergColumnHandle>(
                columnNames.at(i),
                connector::iceberg::IcebergColumnHandle::ColumnType::kRegular,
                columnTypes.at(i),
                columnTypes.at(i)));
      }
    }
    ...
    std::shared_ptr<const connector::iceberg::LocationHandle> locationHandle =
        makeLocationHandle(
            outputDirectoryPath,
            std::nullopt,
            connector::iceberg::LocationHandle::TableType::kNew);
    ...
    return std::make_shared<connector::iceberg::IcebergInsertTableHandle>(
        columnHandles,
        locationHandle,
        schema,
        partitionSpec,
