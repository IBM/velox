...
std::string HiveConnectorSplit::toString() const {
  auto baseString =
     hive_common::ConnectorSplitBase::toStringBase("HiveConnectorSplit");
  if (tableBucketNumber.has_value()) {
    return fmt::format(
        "{} tableBucketNumber: {}]", baseString, tableBucketNumber.value());
  }
  return baseString;
}
...
folly::dynamic HiveConnectorSplit::serialize() const {
  folly::dynamic obj =
     hive_common::ConnectorSplitBase::serializeBase("HiveConnectorSplit");
