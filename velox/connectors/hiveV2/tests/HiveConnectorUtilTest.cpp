...
class HiveConnectorUtilTest : public HiveConnectorTestBase {
 protected:
...
TEST_F(HiveConnectorUtilTest, configureReaderOptions) {
  ...
  auto connectorQueryCtx = std::make_unique<connector::ConnectorQueryCtx>(
      ...
      "");
  auto hiveConfig =
      std::make_shared<hiveV2::HiveConfig>(std::make_shared<config::ConfigBase>(
  ...
  auto createTableHandle = [&]() {
    return std::make_shared<hiveV2::HiveTableHandle>(
        "testConnectorId",
        "testTable",
        false,
  ...
  auto createSplit = [&]() {
    return std::make_shared<hiveV2::HiveConnectorSplit>(
        "testConnectorId",
        "/tmp/",
        fileFormat,
        0UL,
  ...
  customHiveConfigProps[hiveV2::HiveConfig::kLoadQuantum] = "321";
  customHiveConfigProps[hiveV2::HiveConfig::kMaxCoalescedBytes] = "129";
  customHiveConfigProps[hiveV2::HiveConfig::kMaxCoalescedDistance] = "513KB";
  customHiveConfigProps[hiveV2::HiveConfig::kFileColumnNamesReadAsLowerCase] =
      "true";
  customHiveConfigProps[hiveV2::HiveConfig::kOrcUseColumnNames] = "true";
  customHiveConfigProps[hiveV2::HiveConfig::kFooterEstimatedSize] = "1111";
  customHiveConfigProps[hiveV2::HiveConfig::kFilePreloadThreshold] = "9999";
  customHiveConfigProps[hiveV2::HiveConfig::kPrefetchRowGroups] = "10";
  hiveConfig = std::make_shared<hiveV2::HiveConfig>(
...
TEST_F(HiveConnectorUtilTest, cacheRetention) {
  ...
  for (const auto& testData : testSettings) {
    ...
    config::ConfigBase sessionProperties({});
    auto hiveConfig = std::make_shared<hiveV2::HiveConfig>(
    ...
    auto tableHandle = std::make_shared<hiveV2::HiveTableHandle>(
        "testConnectorId",
        "testTable",
        false,
        velox::common::SubfieldFilters{},
    ...
    auto hiveSplit = std::make_shared<hiveV2::HiveConnectorSplit>(
        "testConnectorId",
        "/tmp/",
        FileFormat::DWRF,
        0UL,
...
TEST_F(HiveConnectorUtilTest, configureRowReaderOptions) {
  auto split =
      std::make_shared<hiveV2::HiveConnectorSplit>("", "", FileFormat::UNKNOWN);
