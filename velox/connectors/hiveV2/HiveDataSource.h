...
class HiveDataSource : public hive_common::ConnectorDataSource {
 public:
  HiveDataSource(
  ...
  using WaveDelegateHookFunction =
      std::function<std::shared_ptr<wave::WaveDataSource>(
          const std::shared_ptr<hive_common::TableHandleBase>& hiveTableHandle,
          ...
          folly::Executor* executor,
          const ConnectorQueryCtx* connectorQueryCtx,
          const std::shared_ptr<hive_common::ConnectorConfig>& hiveConfig,
