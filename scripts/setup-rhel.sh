...
function install_build_prerequisites {
  ...
  dnf_install ninja-build cmake gcc-toolset-12 gcc-toolset-14 git wget which bzip2
...
function install_gflags {
  ...
  # RHEL does not include ${INSTALL_PREFIX}/lib in the default ldconfig
...
function install_elfutils-libelf {
  ...
  mkdir -p "${DIR}"
  pushd "${DIR}" || exit
  curl ${CURL_OPTIONS:+${CURL_OPTIONS}} -L "https://sourceware.org/elfutils/ftp/elfutils-latest.tar.bz2" -o ${DIR}.tar.bz2
  tar -xj --strip-components=1 --no-same-owner -f ${DIR}.tar.bz2
  popd || exit
  popd || exit
  (
...
function install_hadoop {
  echo "Installing Hadoop ${HADOOP_VERSION} for HDFS support"
  ...
  mkdir -p "${INSTALL_PREFIX}/hadoop"

  # Copy necessary Hadoop components
  cp -a "${DEPENDENCY_DIR}/hadoop/lib" "${INSTALL_PREFIX}/hadoop/"
  cp -a "${DEPENDENCY_DIR}/hadoop/share" "${INSTALL_PREFIX}/hadoop/"
  cp -a "${DEPENDENCY_DIR}/hadoop/etc" "${INSTALL_PREFIX}/hadoop/"

  echo "Hadoop ${HADOOP_VERSION} installed successfully to ${INSTALL_PREFIX}/hadoop"
}
